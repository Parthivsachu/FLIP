...
const products = [
    {
        id: 1,
        sponsored: true,
        name: "MOTOROLA Razr 60 (PANTONE Lightest Sky, 256 GB)",
        brand: "MOTOROLA",
        rating: 4.6,
        ratingCount: 15344,
        reviewsCount: 1191,
        ...
        upto: "Upto ",
        money: "₹41,000",
        exchange: " Off on Exchange",
        bankOffer: true,
        topDiscount: false,
        image: "https://rukminim2.flixcart.com/image/312/312/xif0q/mobile/2/l/g/-original-imahcqupncz6d9zz.jpeg?q=70"
    },
    {
        id: 2,
        sponsored: true,
        ...
        bankOffer: true,
        topDiscount: false,
        image: "https://rukminim2.flixcart.com/image/312/312/xif0q/mobile/2/z/b/-original-imahcquphydfhcq8.jpeg?q=70"
    },
    {
        id: 3,
        name: "MOTOROLA Edge 60 Fusion 5G (PANTONE Amazonite, 256 GB)",
        brand: "MOTOROLA",
        rating: 4.6,
        ratingCount: 20358,
        reviewsCount: 1191,
        ...
        bankOffer: true,
        topDiscount: true,
        image: "https://rukminim2.flixcart.com/image/312/312/xif0q/mobile/j/x/j/-original-imahbfmdgpjvgecg.jpeg?q=70"
    },
    {
        id: 4,
    ...
    },
    {
        ...
        bankOffer: true,
        topDiscount: true,
        image: "https://rukminim2.flixcart.com/image/312/312/xif0q/mobile/p/t/z/-original-imahbzkz6gyfadqj.jpeg?q=70"
    },
    {
        id: 6,
    ...
    },
    {
        ...
        specs: [
            ...
            "64MP Rear Camera | 13MP Front Camera",
            "5500 mAh Battery",
            "Dimensity 7300 Ultimate Processor",
            "1 Year Manufacturer Warranty on Handset"
        ],
        originalPrice: 18999,
        ...
        bankOffer: true,
        topDiscount: false,
        image: "https://rukminim2.flixcart.com/image/312/312/xif0q/mobile/7/j/m/-original-imahctmxtjhzqzyg.jpeg?q=70"
    },
    {
        id: 8,
        ...
        bankOffer: true,
        topDiscount: true,
        image: "https://rukminim2.flixcart.com/image/312/312/xif0q/mobile/1/4/h/-original-imahcrpwaxrsfjzd.jpeg?q=70"
    },
    {
        id: 9,
        sponsored: false,
        ...
        reviewsCount: 872,
        specs: [
            "8 GB RAM | 256 GB ROM | Expandable Upto 1TB",
            "16.76 cm (6.6 inch) Full HD+ Display",
    ...
    },
    {
        id: 12,
        sponsored: true,
        name: "Google Pixel 8a (Bay, 128 GB)",
        brand: "Google",
        rating: 4.3,
        ratingCount: 6016,
        reviewsCount: 510,
...
function renderProducts(filteredProducts) {
    ...
    if (productsToRender.length === 0) {
        container.innerHTML = '<div>No products match the selected filters.</div>';
        return;
    }

    productsToRender.forEach(product => {
        const productHtml = `
            <div class="mobOne" style="width: 100%;">
                <div class="mobFlex">
                    <div style="width: 100%;">
                        <div class="mobPad">
                            <div class="mobDiv">
                                <div class="mobWidth">
                                    <div class="imgDiv">
                                        <div class="imgBefore"><img class="IMAGE" src="${product.image}" alt="${product.name}"></div>
                                    </div>
                                    <div class="compare">
                                        <div class="compareDiv">
                                            <span class="inline">
                                                <label class="label">
                                                    ...
                                                    <div class="sample" style="transform: translateY(1px);"></div>
                                                </label>
                                            </span>
                                            <label for="" class="addTo">
                                ...
                                <div class="mobDetails">
                                    ...
                                    <div class="priceTag">
                                        <div class="money">
                                            <div>
                                                <div class="offer">${product.discountedPrice.toLocaleString()}</div>
                                                <div class="rupee">${product.originalPrice.toLocaleString()}</div>
                                                <div class="per"><span>${product.discount}</span></div>
                                            </div>
                                            ${product.freeDelivery ? `<div class="freeDiv"><span class="freeDel">Free delivery</span></div>` : ''}
                                        </div>
                                        ${product.assured ? `<div class="assured">
                                            <img height="21" src="https://static-assets-web.flixcart.com/fk-p-linchpin-web/fk-cp-zion/img/fa_9e47c1.png" alt="assured">
                                        </div>` : ''}
                                        ${product.topDiscount ? `
                                            <div class="exchangeMargin"><div class="discountPad"><span class="discountText">Top Discount of the Sale</span></div>` : ''}
                                        <div class="exchangeMargin">
                                            <div class="exchangeDiv">
                                                <div class="upto">${product.upto}</div>
                                                <div class="rate upto">${product.money}</div>
                                                <div class="upto">${product.exchange}</div>
                                            </div>
                                        </div>
                                        ${product.bankOffer ? `<div class="exchangeMargin">
                                            <div class="upto bank">Bank Offer</div>
                                        </div>` : ''}
                                    </div>
                                </div>
                            </div>
...
function setupBrandFilters() {
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', (event) => {
            const brand = event.target.nextElementSibling.nextElementSibling.textContent.trim();
...
function updateClearAllButton() {
    ...
    if (clearButton) {
        clearButton.style.display = (selectedBrands.length > 0 || minPrice !== 0 || maxPrice !== 999999) ? 'inline-block' : 'none';
    }
}
...
function setupClearAllButton() {
    ...
    if (clearButton) {
        clearButton.addEventListener('click', () => {
            ...
            const minPriceSelect = document.querySelector('.min-price');
            const maxPriceSelect = document.querySelector('.max-price');
            if (minPriceSelect) minPriceSelect.value = "0";
            if (maxPriceSelect) maxPriceSelect.value = "999999";
...
function onDrag(point, isMin) {
    return function (event) {
        event.preventDefault();
        const startX = event.clientX || event.touches?.[0].clientX;
        const startLeft = point.offsetLeft;

        function move(e) {
            const currentX = e.clientX || e.touches?.[0].clientX;
            let delta = currentX - startX;
            let newLeft = Math.min(Math.max(startLeft + delta, 0), sliderWidth);

            const otherPoint = isMin ? point2 : point1;
            const otherLeft = parseFloat(otherPoint.style.left || (isMin ? sliderWidth : 0));

            if (isMin && newLeft >= otherLeft - 10) {
                newLeft = otherLeft - 10;
            } else if (!isMin && newLeft <= otherLeft + 10) {
                newLeft = otherLeft + 10;
            }

            newLeft = Math.min(Math.max(newLeft, 0), sliderWidth);
        ...
        function stop() {
            ...
            const value = Math.round((parseFloat(point.style.left) / sliderWidth) * 30000);
            const closest = closestOption((isMin ? minSelect : maxSelect), value);
            if (isMin) {
                minPrice = parseInt(closest);
                minSelect.value = closest;
            } else {
                maxPrice = parseInt(closest);
                maxSelect.value = closest;
            }
            applyFilters();
        }
...
function closestOption(selectEl, value) {
    let closest = selectEl.options[0].value;
    let minDiff = Infinity;
    for (let opt of selectEl.options) {
        const diff = Math.abs(value - parseInt(opt.value));
        if (diff < minDiff) {
            minDiff = diff;
            closest = opt.value;
        }
...
if (point1 && point2 && blueLine) {
    point1.addEventListener('mousedown', onDrag(point1, true));
    point2.addEventListener('mousedown', onDrag(point2, false));
    point1.addEventListener('touchstart', onDrag(point1, true));
...
dotElements.forEach((dot, index) => {
    dot.addEventListener('click', () => {
        if (index >= pricePoints.length - 1) return;
