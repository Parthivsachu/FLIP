const products = [
    ...
    },
    {
        id: 2,
        sponsored: false,
        name: "Google 5G (Geek Black, 128 GB)",
        reviewCount: "",
        assured: false,
        discount: "15%",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    clearTextBtn?.addEventListener('click', () => {
        document.querySelectorAll('.holder[data-price] .checkbox-input').forEach(cb => {
            cb.checked = false;
        ...
        document.querySelectorAll('.holder[data-brand] .checkbox-input').forEach(cb => {
            cb.checked = false;
            const holder = cb.closest('.holder');
            const tick = holder.querySelector('.tick');
            const ticked = holder.querySelector('.ticked');
